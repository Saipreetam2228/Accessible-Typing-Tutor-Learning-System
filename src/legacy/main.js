...
function bootEngine() {
  ...
  let voiceGuidanceEnabled =
    localStorage.getItem("type-tutor-voice-guidance") !== "off";
  let voiceVolume = (function () {
    var stored = parseInt(localStorage.getItem("type-tutor-volume"), 10);
    return isNaN(stored) ? 0.35 : Math.min(1, Math.max(0, stored / 100));
  })();
  let startTime = null;
  ...
  /* Called by fab.js's Voice Guidance switch */
  window._setVoiceGuidance = function (on) {
    voiceGuidanceEnabled = on;
    localStorage.setItem("type-tutor-voice-guidance", on ? "on" : "off");
    if (!on) speechSynthesis.cancel();
  };

  /* Called by fab.js's main Volume slider — keeps voice guidance
     volume in lockstep with the bgm volume level, regardless of
     whether the Sound on/off toggle is muted.                    */
  window._setVoiceVolume = function (level) {
    voiceVolume = Math.min(1, Math.max(0, level));
  };

  /* Called by fab.js's Next Letter Hint switch */
  window._setHintHighlight = function (on) {
    highlightEnabled = on;
    localStorage.setItem("type-tutor-hint-highlight", on ? "on" : "off");
    if (!highlightEnabled) {
      clearNextKeyHighlight();
    } else {
      const correctText = stages[currentStage];
